...
import { GlobalSchema } from '@/page/commande_form/schema'
import { 
    Dialog,
    DialogTrigger,
    DialogContent,
...
const ModifCommandeModal = ({ value, onUpdate }) => {
  ...
  return (
    <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        ...
        <DialogContent className="sm:max-w-3xl [&>button]:bg-red-500 [&>button]:w-8 [&>button]:h-8 [&>button]:flex [&>button]:justify-center [&>button]:items-center [&>button]:rounded-full [&>button]:text-white [&>button]:hover:bg-red-600 [&>button]:hover:cursor-pointer py-10 px-10 bg-white">
            <DialogHeader>
                <DialogTitle className="text-2xl font-bold text-blue-700">Modifier la commande</DialogTitle>
            </DialogHeader>
            <form onSubmit={handleSubmit(submit)}  className="max-h-[70vh] overflow-y-auto p-4 space-y-4">
                ...
                <div className=''>
                    ...
                    {/* telephone */}
                    <div className='flex flex-col space-y-2'>
                        <label htmlFor="telephone" className="font-medium text-gray-700">Telephone :</label>
                        <Input               
                            id="telephone"
                            {...register("telephone")}
                            className="col-span-3 text-gray-800  border-gray-400 rounded-lg h-10"
                        />
                        {errors.telephone && <p className='text-red-500 text-[12px]'>{errors.telephone.message}</p>}
                    </div>
                </div>
                ...
                <DialogFooter className="flex justify-end gap-3 mt-3">
                    ...
                    <Button type="submit" className="bg-indigo-600 hover:bg-indigo-700 text-white rounded-full">Modifier</Button>
                </DialogFooter>
            </form>
        </DialogContent>
    </Dialog>  
...
export default ModifCommandeModal